...
func (d Decimal) NumDigits() int {
	...
	if v.IsInt64() {
		...
		if i64 <= (1<<53) && i64 >= -(1<<53) {
			...
			absVal := math.Abs(float64(i64))
			digits := int(math.Log10(absVal)) + 1
			// math.Log10 can return a value slightly below the true integer for
			// exact powers of 10 (e.g. Log10(1e15) == 14.9999...), which makes
			// int() truncate one digit short. Correct the off-by-one by comparing
			// with the corresponding power of 10.
			pow10 := math.Pow10(digits - 1)
			if absVal >= pow10*10 {
				digits++
			} else if absVal < pow10 {
				digits--
			}
			return digits
		}
