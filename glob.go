...
func (g *glob) globAlts(fsys fs.FS, pattern string, openingIdx, closingIdx int, m []string, firstSegment, beforeMeta bool) (matches []string, err error) {
	...
	for _, d := range dirs {
		// `d` is a real path from the filesystem, so any meta characters in it
		// must be escaped before we embed it in a new glob pattern (via
		// buildAlt), otherwise they'd be interpreted as glob meta characters.
		d = escapeMeta(d)
		patIdx := openingIdx + 1
